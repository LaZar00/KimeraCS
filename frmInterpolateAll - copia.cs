...
namespace KimeraCS
{
    ...
    public partial class frmInterpolateAll : Form
    {
        ...
        public void InterpolateAllAnimations()
        {
            int numTotalAnims, iCounter;
            ...
            if (chkProcessCharLGP.Checked)
            {
                ...
                foreach (stSkeletonAnimationNames itmSkAnim in lstUniqueCharSkAnim) 
                {
                    ...
                    if (fAnimation.numBones == fSkeleton.nBones || fAnimation.numBones == 0)
                    {
                        InterpolateFieldAnimation(ref fSkeleton, ref fAnimation, (int)nudInterpFrameField.Value, false);
                        WriteFieldAnimation(fAnimation, strCharLGPPathDest + "\\" + itmSkAnim.strAnimation);
                    ...
                    if (chkShowAnimsCounter.Checked)
                        progBarIntAllAnim.CreateGraphics().
                                          DrawString(progBarIntAllAnim.Value.ToString("0000") + " / " + numTotalAnims.ToString("0000"), 
                                                     new Font("Arial", (float)12, FontStyle.Bold),
                                                     Brushes.DarkBlue, new PointF(progBarIntAllAnim.Width / 2 - 10, progBarIntAllAnim.Height / 2 - 10));
            ...
            if (chkProcessBattleLGP.Checked && !bCancelPressed)
            {
                ...
                foreach (string itmBattleAnimation in lstbAnimationsPackFiles)
                {
                    ...
                    if (SameBattleAnimNumBones(strbAnimationsPackFullFileName, bSkeleton))
                    {
                        ...
                        InterpolateBattleAnimationsPack(ref bSkeleton, ref bAnimationsPack, (int)nudInterpFrameBattleMagic.Value, false);

                        WriteBattleAnimationsPack(ref bAnimationsPack, strbAnimationsPackFullWriteFileName);
                    ...
                    if (chkShowAnimsCounter.Checked)
                        ...
                        //                             new Font("Arial", (float)12, FontStyle.Bold),
                        //                             Brushes.DarkBlue, new PointF(progBarIntAllAnim.Width / 2 - 10, progBarIntAllAnim.Height / 2 - 10));
                        SetProgressBarText(progBarIntAllAnim,
                                           progBarIntAllAnim.Value.ToString("0000") + " / " + numTotalAnims.ToString("0000"),
                                           ProgressBarTextLocation.Centered,
                                           Color.DarkBlue,
                                           new Font("Arial", (float)12, FontStyle.Bold));

                    //using (Graphics gr = progBarIntAllAnim.CreateGraphics())
                    //{
                    //    gr.DrawString(Text,
